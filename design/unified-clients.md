...
> **Status: implemented in codegen, offline-verified.** Built on the
> module-entrypoint branch (the runtime is inlined, so this needs no engine
> change). The proposal below is preserved as the rationale; this note records
> where the implementation landed and where it diverged.
>
> **What shipped**
>
> - **`_client.ts.gtpl` replaces `_dep.ts.gtpl` + `_augmentations.ts.gtpl`.**
>   Each module renders as a self-contained client: its own root `Client` built
>   from the fields it contributes to `Query`, its own `export const dag`, and a
>   top-level function per root field (`hey.hey()`, `hey.loadHeyFromID(id)`).
