...
`helpers/config-updater` already has the module-flavoured `package-json`,
...
`helpers/config-updater` gains `module-package-json` / `module-tsconfig` /
...
`helpers/config-updater`, golden tests from a captured `introspection.json` +
...
Also keep `config-updater`'s `typescript` pin equal to
...
`configUpdaterBuilder`, and the client helpers this mirrors), `mod.dang`
...
`helpers/config-updater/main.go`, `design/client-gen.md`, `design/client-bundle.md`.
